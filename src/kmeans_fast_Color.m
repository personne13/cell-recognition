...
%% ============================== For RGB colorspace ==================
        ...
        %label_vector = randi([1 no_of_cluster],size(vecred,1),1);
        %label_vector = mod(randperm(size(vecred,1)),no_of_cluster)'+1;
        for i=1:no_of_cluster
            %index=(label_vector==i);
            ...
            meanred1(:,i)=randi([1 ceil(max((vecred(:))))]);%ceil(mean(vecred(index)));
            meangreen1(:,i)=randi([1 ceil(max((vecgreen(:))))]);%ceil(mean(vecgreen(index)));
            meanblue1(:,i)=randi([1 ceil(max((vecblue(:))))]);%ceil(mean(vecblue(index)));
        end
...
%         im = double(im);
        ...
        for i=1:no_of_cluster
            ...
            meana1(:,i)=randi([1 max((veca(:)))]);%ceil(mean(vecred(index)));
            meanb1(:,i)=randi([1 max((vecb(:)))]);%ceil(mean(vecgreen(index)));
